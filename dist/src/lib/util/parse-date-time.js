...
const parse_time_1 = tslib_1.__importDefault(require("./parse-time"));
/**
...
function parseDateTime(dt, culture = '') {
    ...
    if ((dt && dt.includes('.')) || culture === 'DE') {
        ...
        if (parts.length === 2) {
            ...
            // Time
            result.time = (0, parse_time_1.default)(parts[1], pmDesignator);
        }
    }
        ...
        if (parts.length === 2) {
            ...
            if (dtparts.length === 3) {
                if (dtparts[2].length === 4) {
                    ...
                    const isEN = dt.toLowerCase().includes('am') ||
