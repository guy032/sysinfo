...
const detect_split_1 = tslib_1.__importDefault(require("./detect-split"));
/**
...
function parseTime(t, pmDesignator = '') {
    ...
    let hour = 0;
    let min = 0;
    const splitter = (0, detect_split_1.default)(t);
    const parts = t.split(splitter[0].toString());
    if (parts.length >= 2) {
        if (parts[2]) {
            parts[1] += parts[2];
        }
        const isPM = (parts[1] && parts[1].toLowerCase().includes('pm')) ||
