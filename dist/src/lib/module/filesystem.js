...
const _fs_speed = {};
// Helper functions
function getInterfaceType(interfaceType) {
...
function getFsStatsScriptPath() {
    return path_1.default.resolve(__dirname, '../../powershell/fsStats.ps1');
}
// Add function to get path to the fsOpenFiles PowerShell script
function getFsOpenFilesScriptPath() {
    return path_1.default.resolve(__dirname, '../../powershell/fsOpenFiles.ps1');
}
// Add function to get path to the disksIO PowerShell script
function getDisksIOScriptPath() {
    return path_1.default.resolve(__dirname, '../../powershell/disksIO.ps1');
}
/**
 * Calculates filesystem usage
 */
function fsSize(options, callback) {
    const { _linux, _freebsd, _openbsd, _netbsd, _darwin, _sunos, _windows } = (0, platform_1.getPlatformFlagsFromOptions)(options);
    return new Promise((resolve) => {
        process.nextTick(async () => {
            ...
            if (_windows) {
                ...
                for (const line of lines) {
                    if (line !== '') {
                        ...
                        data.push({
                            ...
                            use: lineArray[3] && lineArray[4]
                                ? Number.parseFloat(((100 *
                                    (Number.parseInt(lineArray[4], 10) - Number.parseInt(lineArray[3], 10))) /
                                    Number.parseInt(lineArray[4], 10)).toFixed(2))
                                : 0,
                            mount: lineArray[0],
                            rw: true,
                ...
                const cmd = `df -kPT`;
                (0, child_process_1.exec)(cmd, { maxBuffer: 1024 * 1024 }, (error, stdout) => {
                    if (!error) {
                    ...
                    if (data.length === 0 && _darwin) {
                        const cmd = `df -kPl`;
                        try {
                            const dfdata = (0, child_process_1.execSync)(cmd, { maxBuffer: 1024 * 1024 });
                    ...
                    if (data.length === 0 && _sunos) {
                        // not tested
                        (0, child_process_1.exec)('df -kl', { maxBuffer: 1024 * 1024 }, function (error, stdout) {
                            if (!error) {
                                const lines = stdout.toString().split('\n');
                                for (const line of lines) {
                                    if (line !== '') {
                    ...
                    if (_darwin) {
                        (0, child_process_1.exec)('df -kPl', { maxBuffer: 1024 * 1024 }, function (error, stdout) {
                            if (!error) {
                                const lines = stdout.toString().split('\n');
                                for (const line of lines) {
                                    if (line !== '') {
...
function fsOpenFiles(options, callback) {
    const { _linux, _darwin, _windows } = (0, platform_1.getPlatformFlagsFromOptions)(options);
    return new Promise((resolve) => {
        process.nextTick(async () => {
            const result = {
                max: null,
            ...
            if (_linux) {
                ...
                if (result.allocated === null) {
                    ...
                    try {
                        result.allocated = Number.parseInt((0, child_process_1.execSync)(cmd, { maxBuffer: 1024 * 1024 }).toString(), 10);
                    }
                    catch {
                        util.noop();
                    }
            ...
            if (_darwin) {
                ...
                try {
                    const lines = (0, child_process_1.execSync)(cmd, { maxBuffer: 1024 * 1024 })
                        .toString()
                        .split('\n');
                    for (const line of lines) {
    ...
    return new Promise((resolve) => {
        process.nextTick(async () => {
            ...
            if (_linux) {
                ...
                try {
                    let parts = { blockdevices: [] };
                    const data = (0, child_process_1.execSync)(cmd, { maxBuffer: 1024 * 1024 }).toString();
                    try {
                ...
                if (result.length === 0) {
                    ...
                    try {
                        for (const line of (0, child_process_1.execSync)(cmd, { maxBuffer: 1024 * 1024 })
                            .toString()
                            .trim()
                            .split('\n')) {
            ...
            if (_darwin) {
                try {
                    const lines = (0, child_process_1.execSync)('diskutil info -all', { maxBuffer: 1024 * 1024 })
                        .toString()
                        .split('\n');
                    let diskGroup = [];
                    let lineArray = [];
                    ...
                    for (const line of lines) {
                        if (!line.startsWith(' ') &&
                            ...
                            const status = diskGroup.join('\n').includes('SMART Status')
                                ...
                                : 'unknown';
                            const type = MediaName.includes('SSD') ||
                            ...
                            const partitions = diskGroup.join('\n').includes('Partition Count: ')
                                ...
                                : '';
                            if (deviceName) {
                                const isHD = deviceName.startsWith('/dev/disk') &&
                                    partitions &&
                                    diskGroup.join('\n').includes('Internal: ');
                        ...
                        if ((line.startsWith('/dev/disk') && !line.trim().startsWith('/dev/disk0s')) ||
                            diskGroup.length > 0) {
                            diskGroup.push(line);
                        }
                    }
            ...
            if (_windows) {
                try {
                    ...
                    if (Object.keys(diskInfo).length > 0) {
                        const deviceId = diskInfo.DeviceID?.replace(/\\\\.\\|\\\\?\\|\\\\.\\PhysicalDrive/g, '').trim() || '';
                        ...
                        if (size > 0) {
                            const diskLayoutEntry = {
                                device: deviceId,
                                type: mediaType?.toLowerCase().includes('ssd') ||
                                ...
                                smartStatus: diskInfo.Status?.trim() === 'OK' ? 'Ok' : diskInfo.Status?.trim() || 'unknown',
...
function fsStats(options, callback) {
    const { _linux, _windows, _freebsd, _openbsd, _netbsd, _sunos } = (0, platform_1.getPlatformFlagsFromOptions)(options);
    return new Promise((resolve) => {
        process.nextTick(async () => {
