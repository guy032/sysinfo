...
function ifaceListLinux() {
    ...
    try {
        const all = (0, child_process_1.execSync)(cmd, util.execOptsLinux)
            .toString()
            .split('\n')
            .map((line) => line.trim())
            .join('\n');
        ...
        try {
            const all = (0, child_process_1.execSync)('nmcli -t -f general,wifi-properties,wired-properties,interface-flags,capabilities,nsp device show 2>/dev/null', util.execOptsLinux).toString();
            const parts = all.split('\n\n');
            let i = 1;
            for (const ifaceDetails of parts) {
                const lines = ifaceDetails.split('\n');
...
function nmiDeviceLinux(iface) {
    ...
    try {
        const lines = (0, child_process_1.execSync)(cmd, util.execOptsLinux).toString().split('\n');
...
function getWifiNetworkListNmi() {
    ...
    try {
        const stdout = (0, child_process_1.execSync)(cmd, util.execOptsLinux);
...
function getWifiNetworkListIw(iface) {
    const result = [];
    try {
        const iwlistParts = (0, child_process_1.execSync)(`export LC_ALL=C; iwlist ${iface} scan 2>&1; unset LC_ALL`, util.execOptsLinux)
            .toString()
            .split('        Cell ');
    ...
    return new Promise((resolve) => {
        process.nextTick(() => {
            ...
            if (platformFlags._linux) {
                ...
                if (result.length === 0) {
                    try {
                        const iwconfigParts = (0, child_process_1.execSync)('export LC_ALL=C; iwconfig 2>/dev/null; unset LC_ALL', util.execOptsLinux)
                            .toString()
                            .split('\n\n');
                        let iface = '';
                ...
                (0, child_process_1.exec)(cmd, { maxBuffer: 1024 * 40_000 }, function (error, stdout) {
