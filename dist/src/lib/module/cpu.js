...
const amd_base_frequencies_1 = require("../../mapping/amd-base-frequencies");
const socket_types_1 = require("../../mapping/socket-types");
const socket_types_by_name_1 = require("../../mapping/socket-types-by-name");
...
function getSocketTypesByName(str) {
    let result = '';
    for (const key of Object.keys(socket_types_by_name_1.socketTypesByName)) {
        const names = socket_types_by_name_1.socketTypesByName[key].split(' ');
...
function getAMDSpeed(brand) {
    let result = '0';
    for (const key in amd_base_frequencies_1.AMDBaseFrequencies) {
        if (Object.prototype.hasOwnProperty.call(amd_base_frequencies_1.AMDBaseFrequencies, key)) {
            ...
            if (found === parts.length) {
                result = amd_base_frequencies_1.AMDBaseFrequencies[key];
            }
        }
    }
...
function getCpuCurrentSpeedSync(options) {
    const { _linux } = (0, platform_1.getPlatformFlagsFromOptions)(options);
    const cpus = os.cpus();
    ...
    if (cpus && cpus.length > 0 && cpus[0].speed) {
        ...
        try {
            const speedStrings = (0, child_process_1.execSync)('cat /proc/cpuinfo | grep "cpu MHz" | cut -d " " -f 3', util.execOptsLinux)
                .toString()
                .split('\n')
...
function cpuFlags(options = {}, callback) {
    const { _windows, _linux, _darwin, _freebsd, _openbsd, _netbsd, _sunos } = (0, platform_1.getPlatformFlagsFromOptions)(options);
    return new Promise((resolve) => {
        process.nextTick(() => {
            ...
            if (_linux) {
                try {
                    (0, child_process_1.exec)('export LC_ALL=C; lscpu; unset LC_ALL', function (error, stdout) {
                        if (!error) {
...
function getCpu(options = {}) {
    ...
    return new Promise((resolve) => {
        process.nextTick(async () => {
            ...
            if (_darwin) {
                (0, child_process_1.exec)('sysctl machdep.cpu hw.cpufrequency_max hw.cpufrequency_min hw.packages hw.physicalcpu_max hw.ncpu hw.tbfrequency hw.cpufamily hw.cpusubfamily', function (error, stdout) {
                    const lines = stdout.toString().split('\n');
                    ...
                    if (os.arch() === 'arm64') {
                        result.socket = 'SOC';
                        try {
                            const clusters = (0, child_process_1.execSync)('ioreg -c IOPlatformDevice -d 3 -r | grep cluster-type')
                                .toString()
                                .split('\n');
            ...
            if (_linux) {
                ...
                (0, child_process_1.exec)('export LC_ALL=C; lscpu; echo -n "Governor: "; cat /sys/devices/system/cpu/cpu0/cpufreq/scaling_governor 2>/dev/null; echo; unset LC_ALL', function (error, stdout) {
                    if (!error) {
                    ...
                    // socket type
                    let lines2 = [];
                    (0, child_process_1.exec)('export LC_ALL=C; dmidecode –t 4 2>/dev/null | grep "Upgrade: Socket"; unset LC_ALL', function (error2, stdout2) {
            ...
            if (_freebsd || _openbsd || _netbsd) {
                ...
                (0, child_process_1.exec)('export LC_ALL=C; dmidecode -t 4; dmidecode -t 7 unset LC_ALL', function (error, stdout) {
                    let cache = [];
                    let lines = [];
                    if (!error) {
            ...
            if (_windows) {
                try {
                    ...
                    Promise.all(workload).then((data) => {
                        ...
                        if (socket_types_1.socketTypes[socketId]) {
                            result.socket = socket_types_1.socketTypes[socketId];
                        }
 ...
 */
function cpuCurrentSpeed(options = {}, callback) {
    return new Promise((resolve) => {
        process.nextTick(() => {
