...
function getNvidiaSmi() {
    ...
    if (_windows) {
        try {
            const basePath = util_1.WINDIR + '\\System32\\DriverStore\\FileRepository';
            // find all directories that have an nvidia-smi.exe file
            const candidateDirs = (0, fs_1.readdirSync)(basePath).filter((dir) => (0, fs_1.readdirSync)([basePath, dir].join('/')).includes('nvidia-smi.exe'));
            // use the directory with the most recently created nvidia-smi.exe file
            let targetDir = '';
            let maxTime = 0;
            for (const currentDir of candidateDirs) {
                const nvidiaSmi = (0, fs_1.statSync)([basePath, currentDir, 'nvidia-smi.exe'].join('/'));
...
function nvidiaSmi(options) {
    ...
    options = options || util_1.execOptsWin;
    if (nvidiaSmiExe) {
        ...
        try {
            return (0, child_process_1.execSync)(cmd, options).toString();
        }
        catch {
            (0, util_1.noop)();
        }
...
function parseLinesDarwin(graphicsArr) {
    ...
    try {
        for (const item of graphicsArr) {
            ...
            const vram = (Number.parseInt(item.spdisplays_vram || '', 10) || 0) *
                ((item.spdisplays_vram || '').includes('GB') ? 1024 : 1);
            const vramDyn = (Number.parseInt(item.spdisplays_vram_shared || '', 10) || 0) *
            ...
            if (item.spdisplays_ndrvs && item.spdisplays_ndrvs.length > 0) {
                for (const displayItem of item.spdisplays_ndrvs) {
                    ...
                    const serial = displayItem['_spdisplays_display-serial-number'] ||
                        displayItem['_spdisplays_display-serial-number2'] ||
                        null;
                    res.displays.push({
                        vendor: getVendorFromId(displayItem['_spdisplays_display-vendor-id'] || '') ||
                        ...
                        currentResX: currentResolution.length > 1 ? Number.parseInt(currentResolution[0], 10) : null,
                        currentResY: currentResolution.length > 1 ? Number.parseInt(currentResolution[1], 10) : null,
                        positionX: 0,
                        positionY: 0,
                        currentRefreshRate: currentResolutionParts.length > 1
...
function parseLinesLinuxControllers(lines) {
    ...
    let pciIDs = [];
    try {
        pciIDs = (0, child_process_1.execSync)('export LC_ALL=C; dmidecode -t 9 2>/dev/null; unset LC_ALL | grep "Bus Address: "', util_1.execOptsLinux)
            .toString()
            .split('\n');
...
function parseLinesLinuxEdid(edid) {
    ...
    result.resolutionX = Number.parseInt('0x0' + edid.slice(start + 8, start + 9) + edid.slice(start + 4, start + 6), 10);
    result.resolutionY = Number.parseInt('0x0' + edid.slice(start + 14, start + 15) + edid.slice(start + 10, start + 12), 10);
    result.sizeX = Number.parseInt('0x0' + edid.slice(start + 28, start + 29) + edid.slice(start + 24, start + 26), 10);
    result.sizeY = Number.parseInt('0x0' + edid.slice(start + 29, start + 30) + edid.slice(start + 26, start + 28), 10);
    // monitor name
        ...
        if (lines[i].trim() !== '') {
            ...
            if (lines[i].toLowerCase().includes('*current')) {
                ...
                if (parts1 && parts1.length > 1 && parts1[0].includes('x')) {
                    ...
                    currentDisplay.currentResX = (0, util_1.toInt)(resParts[0]);
                    currentDisplay.currentResY = (0, util_1.toInt)(resParts[1]);
                }
                is_current = true;
            }
            if (is_current &&
                ...
                if (parts1 && parts1.length > 1 && parts1[1].toLowerCase().includes('hz')) {
                    currentDisplay.currentRefreshRate = (0, util_1.toInt)(parts1[1]);
                }
                is_current = false;
            }
        }
 ...
 */
function getGraphicsScriptPath() {
    // Calculate the path to the PowerShell script relative to this file
    return path_1.default.resolve(__dirname, '../../powershell/graphics.ps1');
}
/**
...
function graphics(options = {}, callback) {
    ...
    return new Promise((resolve) => {
        process.nextTick(() => {
                ...
                (0, child_process_1.exec)(cmd, function (error, stdout) {
                    if (!error) {
                        try {
                            ...
                            result = parseLinesDarwin((0, util_1.plistParser)(output)[0]._items);
                        }
                        catch {
                            (0, util_1.noop)();
                        }
                ...
                (0, child_process_1.exec)(cmd, function (error, stdout) {
                    if (!error) {
                        ...
                        if (result.controllers.length === 0) {
                            ...
                            // Match by busAddress
                            result.controllers = result.controllers.map((controller) => mergeControllerNvidia(controller, nvidiaData.find((contr) => contr.pciBus
                                ?.toLowerCase()
                    ...
                    (0, child_process_1.exec)(clInfoCmd, function (error, stdout) {
                        if (!error) {
                            const lines = stdout.toString().split('\n');
                            result.controllers = parseLinesLinuxClinfo(result.controllers, lines);
                        }
                        // Get display depth
                        const depthCmd = "xdpyinfo 2>/dev/null | grep 'depth of root window' | awk '{ print $5 }'";
                        (0, child_process_1.exec)(depthCmd, function (error, stdout) {
                            let depth = 0;
                            if (!error) {
                                const lines = stdout.toString().split('\n');
                            ...
                            (0, child_process_1.exec)(displayCmd, function (error, stdout) {
                                if (!error) {
                                    const lines = stdout.toString().split('\n');
                ...
                try {
                    ...
                    util
                        ...
                        if (output && output.trim()) {
                            ...
                            console.warn('Output was empty or only whitespace');
                        }
            ...
            else if (platformFlags._freebsd ||
