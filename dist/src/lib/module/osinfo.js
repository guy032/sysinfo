...
exports.uuid = exports.osInfo = void 0;
...
const { _platform, _linux, _darwin, _windows, _freebsd, _openbsd, _netbsd, _sunos } = (0, platform_1.getPlatformFlags)();
// --------------------------
...
function getLogoFile(options = {}, distro) {
    ...
    let result = platformFlags._platform;
    if (platformFlags._windows) {
        result = 'windows';
    }
    ...
    return new Promise((resolve) => {
        process.nextTick(() => {
            fs.stat('/sys/firmware/efi', function (err) {
                ...
                (0, child_process_1.exec)('dmesg | grep -E "EFI v"', function (error, stdout) {
                    if (!error) {
...
const isUefiWindows = function () {
    return new Promise((resolve) => {
        process.nextTick(() => {
            try {
                (0, child_process_1.exec)('findstr /C:"Detected boot environment" "%windir%\\Panther\\setupact.log"', util.execOptsWin, function (error, stdout) {
                    if (!error) {
                        const line = stdout.toString().split('\n\r')[0];
                        return resolve(line.toLowerCase().includes('efi'));
                    }
                    (0, child_process_1.exec)('echo %firmware_type%', util.execOptsWin, function (error, stdout) {
                        if (!error) {
...
async function getUniqueMacAddresses(options = {}) {
    let macs = [];
    const platformFlags = (0, platform_1.getPlatformFlagsFromOptions)(options);
...
const uuid = function (options = {}, callback) {
    const platformFlags = (0, platform_1.getPlatformFlagsFromOptions)(options);
    return new Promise((resolve) => {
        process.nextTick(async () => {
            ...
            if (platformFlags._darwin) {
                (0, child_process_1.exec)('system_profiler SPHardwareDataType -json', function (error, stdout) {
                    if (!error) {
                        try {
...
echo -n "hardware: "; cat /sys/class/dmi/id/product_uuid 2> /dev/null; echo;`;
            ...
            if (platformFlags._freebsd || platformFlags._openbsd || platformFlags._netbsd) {
                (0, child_process_1.exec)('sysctl -i kern.hostid kern.hostuuid', function (error, stdout) {
                    const lines = stdout.toString().split('\n');
            ...
            if (platformFlags._windows) {
                ...
                util
                    ...
                    util
                        .powerShell(`${sysdir}\\reg query "HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Cryptography" /v MachineGuid`, options)
                        .then((stdout) => {
                        const stdoutStr = Array.isArray(stdout) ? stdout.join('\n') : stdout;
...
// --------------------------
// OS Information
const osInfo = function (options = {}, callback) {
    const platformFlags = (0, platform_1.getPlatformFlagsFromOptions)(options);
    return new Promise((resolve) => {
        process.nextTick(() => {
            const result = {
            ...
            if (platformFlags._linux) {
                (0, child_process_1.exec)('cat /etc/*-release; cat /usr/lib/os-release; cat /etc/openwrt_release', function (error, stdout) {
                    const release = {};
                    const lines = stdout.toString().split('\n');
                    for (const line of lines) {
                        if (line.includes('=')) {
            ...
            if (platformFlags._freebsd || platformFlags._openbsd || platformFlags._netbsd) {
                (0, child_process_1.exec)('sysctl kern.ostype kern.osrelease kern.osrevision kern.hostuuid machdep.bootmethod kern.geom.confxml', function (error, stdout) {
                    const lines = stdout.toString().split('\n');
            ...
            if (platformFlags._darwin) {
                (0, child_process_1.exec)('sw_vers; sysctl kern.ostype kern.osrelease kern.osrevision kern.uuid', function (error, stdout) {
                    const lines = stdout.toString().split('\n');
                    result.serial = util.getValue(lines, 'kern.uuid');
                    result.distro = util.getValue(lines, 'ProductName');
                    result.release = (util.getValue(lines, 'ProductVersion', ':', true, true) +
                        ' ' +
                        util.getValue(lines, 'ProductVersionExtra', ':', true, true)).trim();
            ...
            if (platformFlags._sunos) {
                ...
                (0, child_process_1.exec)('uname -o', function (error, stdout) {
                    const lines = stdout.toString().split('\n');
            ...
            if (platformFlags._windows) {
                ...
                try {
                    ...
                    const scriptPath = path_1.default.resolve(__dirname, '../../powershell/osinfo.ps1');
 ...
 */
async function fetchAllApplications(options = {}) {
    const allApps = [];
    let skip = 0;
    const batchSize = 100;
    let totalCount = 0;
    ...
    // Get the script path
    const scriptPath = path_1.default.resolve(__dirname, '../../powershell/applications.ps1');
...
function applications(options = {}, callback) {
    const platformFlags = (0, platform_1.getPlatformFlagsFromOptions)(options);
    return new Promise((resolve) => {
        process.nextTick(async () => {
            const apps = [];
    ...
    return new Promise((resolve) => {
        process.nextTick(() => {
            if (platformFlags._windows) {
                ...
                let result = '';
                (0, child_process_1.exec)('echo $SHELL', function (error, stdout) {
                    if (!error) {
