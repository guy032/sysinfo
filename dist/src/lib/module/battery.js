...
function getBatteryScriptPath() {
    ...
    return path_1.default.resolve(__dirname, '../../powershell/battery.ps1');
}
/**
 ...
 */
function battery(options = {}, callback) {
    ...
    return new Promise((resolve) => {
        process.nextTick(() => {
            ...
            if (platform._linux) {
                ...
                (0, child_process_1.exec)('sysctl -i hw.acpi.battery hw.acpi.acline', (error, stdout) => {
                    if (!error) {
                        const lines = stdout.toString().split('\n');
                ...
                (0, child_process_1.exec)('ioreg -n AppleSmartBattery -r | egrep "CycleCount|IsCharging|DesignCapacity|MaxCapacity|CurrentCapacity|DeviceName|BatterySerialNumber|Serial|TimeRemaining|Voltage"; pmset -g batt | grep %', (error, stdout) => {
                    if (stdout) {
                        const lines = stdout
                            .toString()
                            .replaceAll(/ +/g, '')
