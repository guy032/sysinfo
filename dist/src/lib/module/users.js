        ...
        if (line === '---') {
            ...
            if (isWhopart) {
                ...
                if (wFirst) {
                    ...
                    whoLine = resultWho.filter((obj) => obj.user.slice(0, 8).trim() === resultW.user && obj.tty === resultW.tty);
                    if (whoLine.length === 1) {
                        result.push({
                            user: whoLine[0].user,
                            tty: whoLine[0].tty,
...
function parseUsersDarwin(lines) {
    ...
    for (const line of lines) {
        if (line === '---') {
            ...
            // who part
            if (isWhopart) {
                const dt = String(new Date().getFullYear()) +
                    '-' +
                    ('0' +
                        ('JANFEBMARAPRMAYJUNJULAUGSEPOCTNOVDEC'.indexOf(l[2].toUpperCase()) / 3 + 1)).slice(-2) +
                    '-' +
                    ('0' + l[3]).slice(-2);
                try {
                    if (new Date(dt) > new Date()) {
                        const adjustedDt = String(new Date().getFullYear() - 1) +
                            '-' +
                            ('0' +
                                ('JANFEBMARAPRMAYJUNJULAUGSEPOCTNOVDEC'.indexOf(l[2].toUpperCase()) / 3 + 1)).slice(-2) +
                            '-' +
                ...
                // find corresponding 'who' line
                whoLine = resultWho.filter((obj) => obj.user.slice(0, 10) === resultW.user?.slice(0, 10) &&
...
function users(options = {}, callback) {
    ...
    return new Promise((resolve) => {
        process.nextTick(() => {
            ...
            // Linux
            if (platform._linux) {
                (0, child_process_1.exec)('export LC_ALL=C; who --ips; echo "---"; w; unset LC_ALL | tail -n +2', (error, stdout) => {
                    if (error) {
                        if (callback) {
                            callback(result);
                        }
                        ...
                        if (parsed.length === 0) {
                            (0, child_process_1.exec)('who; echo "---"; w | tail -n +2', (error, stdout) => {
                                if (error) {
                                    if (callback) {
                                        callback(result);
                                    }
            ...
            if (platform._freebsd || platform._openbsd || platform._netbsd) {
                (0, child_process_1.exec)('who; echo "---"; w -ih', (error, stdout) => {
                    if (error) {
                        if (callback) {
                            callback(result);
                        }
            ...
            // SunOS
            if (platform._sunos) {
                (0, child_process_1.exec)('who; echo "---"; w -h', (error, stdout) => {
                    if (error) {
                        if (callback) {
                            callback(result);
                        }
            ...
            // Darwin (macOS)
            if (platform._darwin) {
                (0, child_process_1.exec)('export LC_ALL=C; who; echo "---"; w -ih; unset LC_ALL', (error, stdout) => {
                    if (error) {
                        if (callback) {
                            callback(result);
                        }
            ...
            if (platform._windows) {
                try {
                    ...
                    const getSessionsBatched = (skip) => new Promise((resolveSession) => {
                    ...
                    const getLoggedOnUsersBatched = (skip) => new Promise((resolveLogged) => {
                            ...
                            util.powerShell(explorerCmd, options).then((explorerData) => {
                                ...
                                util.powerShell('query user', options).then((queryUserData) => {
                                    ...
                                    if (allSessions.length > 0 && allLoggedOnUsers.length > 0) {
                                        ...
                                        const queryUsers = parseWinUsersQuery(typeof queryUserData === 'string'
                                            ? queryUserData.split('\r\n')
                                        ...
                                        try {
                                            ...
                                            users = parseWinUsers(typeof userProcessData === 'string'
                                                ? userProcessData.split(/\n\s*\n/)
                                                : userProcessData || [], queryUsers);
                                        }
                                        ...
                                        for (const user of users) {
                                            ...
                                            for (const id in loggedons) {
                                                if (Object.prototype.hasOwnProperty.call(loggedons, id)) {
                                                    ...
                                                    if (loggedonUser.user === user.user ||
                                                        ...
                                                        allUserSessions.push({
                                                            ...
                                                            // Calculate duration
                                                            duration: calculateDuration(sessionData && sessionData.startTime ? sessionData.startTime : '', sessionData && sessionData.endTime ? sessionData.endTime : ''),
