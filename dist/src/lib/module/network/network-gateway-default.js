    ...
    return new Promise((resolve) => {
        process.nextTick(() => {
            ...
            if (platform._linux || platform._freebsd || platform._openbsd || platform._netbsd) {
                ...
                try {
                    (0, child_process_1.exec)(cmd, { maxBuffer: 1024 * 20_000 }, function (error, stdout) {
                        if (!error) {
                ...
                try {
                    (0, child_process_1.exec)(cmd, { maxBuffer: 1024 * 20_000 }, function (error, stdout) {
                        if (error) {
                            (0, child_process_1.exec)('netstat -rn', { maxBuffer: 1024 * 20_000 }, function (error, stdout) {
                                if (!error) {
                                    const lines = stdout
                                        .toString()
                                        .split('\n')
                ...
                util
                    .powerShell('Get-NetRoute -DestinationPrefix 0.0.0.0/0 | Select-Object -ExpandProperty NextHop', options)
                    .then((stdout) => {
                    const lines = stdout.toString().split('\r\n');
                    for (const line of lines) {
                        if (line && line.trim()) {
