    ...
    return new Promise((resolve) => {
        process.nextTick(() => {
            ...
            if (!_network[ifaceSanitized] ||
                ...
                if (platform._linux) {
                    if (fs_1.default.existsSync('/sys/class/net/' + ifaceSanitized)) {
                        cmd =
                            'cat /sys/class/net/' +
                                ifaceSanitized +
                                '/operstate; ' +
                ...
                if (platform._windows) {
                    ...
                    const getNetworkPerformanceData = async () => {
                        const stdout = await util.powerShell(`Get-CimInstance Win32_PerfRawData_Tcpip_NetworkInterface | Where-Object { $_.Name -match '${ifaceSanitized.replaceAll(/[^\dA-Za-z]/g, '.')}' } | Select-Object Name,BytesReceivedPersec,PacketsReceivedErrors,PacketsReceivedDiscarded,BytesSentPersec,PacketsOutboundErrors,PacketsOutboundDiscarded | fl`, options);
                    ...
                    measureNetworkSpeed().then((measuredResult) => {
                        if (measuredResult) {
                            ...
                            (0, network_interfaces_1.networkInterfaces)(options).then((interfaces) => {
                                ...
                                for (const det of interfaceArray) {
                                    ...
                                    const isMatch = det.iface.toLowerCase() === ifaceSanitized.toLowerCase() ||
...
async function networkStats(options = {}, callback) {
    return new Promise((resolve) => {
        process.nextTick(async () => {
            const interfaces = await (0, network_interfaces_1.networkInterfaces)(options);
            const results = await Promise.all(interfaces.map((iface) => networkStatsSingle(options, iface.iface)));
            if (callback) {
                callback(results);
            }
            resolve(results);
