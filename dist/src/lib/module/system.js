    ...
    return new Promise((resolve) => {
        process.nextTick(() => {
            ...
            if (platform._linux || platform._freebsd || platform._openbsd || platform._netbsd) {
                (0, child_process_1.exec)('export LC_ALL=C; dmidecode -t system 2>/dev/null; unset LC_ALL', function (error, stdout) {
            ...
            echo -n "sys_vendor: "; cat /sys/devices/virtual/dmi/id/sys_vendor 2>/dev/null; echo;`;
                    try {
                        lines = (0, child_process_1.execSync)(cmd, util.execOptsLinux).toString().split('\n');
                    ...
                    if (!result.virtual) {
                        try {
                            const disksById = (0, child_process_1.execSync)('ls -1 /dev/disk/by-id/ 2>/dev/null', util.execOptsLinux).toString();
                    ...
                    if ((platform._freebsd || platform._openbsd || platform._netbsd) &&
                        !result.virtualHost) {
                        try {
                            const procInfo = (0, child_process_1.execSync)('dmidecode -t 4', util.execOptsLinux);
                    ...
                    try {
                        ...
                        const stdout = (0, child_process_1.execSync)(cmd, util.execOptsLinux).toString();
            ...
            if (platform._darwin) {
                (0, child_process_1.exec)('ioreg -c IOPlatformExpertDevice -d 2', function (error, stdout) {
                    if (!error) {
