...
import { execOptsLinux, execOptsWin, isRaspberry, noop, plistParser, toInt, WINDIR, } from '../util';
...
function getNvidiaSmi() {
    ...
    if (_windows) {
        try {
            ...
            const candidateDirs = readdirSync(basePath).filter((dir) => readdirSync([basePath, dir].join('/')).includes('nvidia-smi.exe'));
...
function parseLinesLinuxControllers(lines) {
    ...
    let pciIDs = [];
    try {
        pciIDs = execSync('export LC_ALL=C; dmidecode -t 9 2>/dev/null; unset LC_ALL | grep "Bus Address: "', execOptsLinux)
            .toString()
            .split('\n');
 ...
 */
export function graphics(options = {}, callback) {
