/**
 * Apple model information interface
 */
interface AppleModelInfo {
  key: string;
  name: string;
  size: string;
  processor: string;
...
/**
 * Apple model result interface
 */
interface AppleModelResult {
  key: string;
  model: string;
  version: string;
}

export default function getAppleModel(key: string): AppleModelResult {
  const appleModelIds: AppleModelInfo[] = [
    {
      key: 'Mac15,12',
      name: 'MacBook Air',
      size: '13-inch',
  ...
  if (Number(list[0].size) > 0 && list[0].size.toString().trim() !== '') {
