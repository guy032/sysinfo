 ...
 */
function getBatteryScriptPath(): string {
...
// Interface for Windows battery data parsing
interface IWinBatteryPart extends Partial<IBatteryData> {
  status?: number;
}
...
function handleResult(
  result: IBatteryData,
  resolve: (value: IBatteryData) => void,
  callback?: IBatteryCallback,
): void {
  if (callback) {
    callback(result);
  }
...
function parseWinBatteryPart(
  ...
  if (status >= '0') {
    ...
    result.designedCapacity = Number.parseInt(
      util.getValue(lines, 'DesignCapacity', ':') || designedCapacity.toString(),
      10,
    );
    result.voltage = Number.parseInt(util.getValue(lines, 'DesignVoltage', ':') || '0', 10) / 1000;
    result.capacityUnit = 'mWh';
    result.percent = Number.parseInt(
      util.getValue(lines, 'EstimatedChargeRemaining', ':') || '0',
      10,
    );
    result.currentCapacity = Math.round(
      (Number(result.maxCapacity) * Number(result.percent)) / 100,
    );
    result.isCharging =
    ...
    process.nextTick(() => {
      ...
      if (platform._linux) {
        ...
        if (battery_path) {
          fs.readFile(battery_path + 'uevent', (error, stdout) => {
            if (error) {
              ...
              result.cycleCount = Number.parseInt(
                '0' + util.getValue(lines, 'POWER_SUPPLY_CYCLE_COUNT', '='),
                10,
              );
              result.maxCapacity = Math.round(
                (Number.parseInt(
                  '0' + util.getValue(lines, 'POWER_SUPPLY_CHARGE_FULL', '=', true, true),
                  10,
                ) /
                  1000) *
                  (result.voltage || 1),
              ...
              const desingedMinVoltage =
                Number.parseInt(
                  '0' + util.getValue(lines, 'POWER_SUPPLY_VOLTAGE_MIN_DESIGN', '='),
                  10,
                ) / 1_000_000;
              result.designedCapacity = Math.round(
                (Number.parseInt(
                  '0' + util.getValue(lines, 'POWER_SUPPLY_CHARGE_FULL_DESIGN', '=', true, true),
                  10,
                ) /
                  1000) *
              ...
              result.currentCapacity = Math.round(
                (Number.parseInt('0' + util.getValue(lines, 'POWER_SUPPLY_CHARGE_NOW', '='), 10) /
                  1000) *
                  (result.voltage || 1),
              );
              ...
              if (!result.maxCapacity) {
                result.maxCapacity =
                  Number.parseInt(
                    '0' + util.getValue(lines, 'POWER_SUPPLY_ENERGY_FULL', '=', true, true),
                    10,
                  ) / 1000;
                result.designedCapacity =
                  Number.parseInt(
                    '0' + util.getValue(lines, 'POWER_SUPPLY_ENERGY_FULL_DESIGN', '=', true, true),
                    10,
                  ) / 1000 || result.maxCapacity;
        ...
        exec('sysctl -i hw.acpi.battery hw.acpi.acline', (error, stdout) => {
          if (!error) {
            const lines = stdout.toString().split('\n');
            const batteries = Number.parseInt(
        ...
        exec(
          ...
          (error, stdout) => {
            if (stdout) {
              ...
              result.capacityUnit = result.voltage ? 'mWh' : 'mAh';
              result.maxCapacity = Math.round(
                Number.parseInt('0' + util.getValue(lines, 'applerawmaxcapacity', '='), 10) *
                  (result.voltage || 1),
              );
              result.currentCapacity = Math.round(
                Number.parseInt('0' + util.getValue(lines, 'applerawcurrentcapacity', '='), 10) *
                  (result.voltage || 1),
              );
              result.designedCapacity = Math.round(
                Number.parseInt('0' + util.getValue(lines, 'DesignCapacity', '='), 10) *
                  (result.voltage || 1),
              );
              ...
              try {
                ...
                if (batteryData && batteryData.BatteryData) {
                  ...
                  if (batteries.length > 0) {
                    let first = false;
                    const additionalBatteries: IBatteryData[] = [];

                    for (const [i, battery] of batteries.entries()) {
                      const lines = Object.entries(battery).map(
                        ([key, value]) => `${key} : ${value}`,
                      );
                      const designedCapacity =
                        designCapacities && designCapacities.length >= i + 1 && designCapacities[i]
                          ? util.toInt(designCapacities[i] as string)
                          : 0;
                      const fullChargeCapacity =
                        fullChargeCapacities &&
                        fullChargeCapacities.length >= i + 1 &&
                        fullChargeCapacities[i]
                          ? util.toInt(fullChargeCapacities[i] as string)
                          : 0;
