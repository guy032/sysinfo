 ...
 */
function createWinRMParams(opts: IWinRMOptions): IWinRMParams {
 ...
 */
async function cleanupShell(winrm: any, params: IWinRMParams): Promise<void> {
  if (!params.shellId) {
    return;
  }
...
async function executeInShell(
  ...
  try {
    // Create the shell
    const shellResult = await withTimeout<string>(
      winrm.shell.doCreateShell(params),
      shellTimeout,
      'Shell creation',
    );
    ...
    const execResult = await withTimeout<string>(
      winrm.command.doExecuteCommand(params),
      cmdTimeout,
      'Command execution',
    );
    ...
    // Get the output
    const outputResult = await withTimeout<string>(
      winrm.command.doReceiveOutput(params),
      cmdTimeout,
      'Receive output',
    );
    ...
    console.error(
      `[WinRM] Error in shell ${params.shellId || 'unknown'}: ${error.message} after ${
        Date.now() - startTime
      }ms`,
    );
 ...
 */
export async function winRMSingleShell(cmds: string[], opts: IWinRMOptions): Promise<string[]> {
  ...
  try {
    ...
    const shellResult = await withTimeout<string>(
      opts.winrm.shell.doCreateShell(params),
      shellTimeout,
      'Shell creation',
    );

    if (shellResult.timedOut || shellResult.error || !shellResult.result) {
      return Array.from({ length: cmds.length }).fill('') as string[];
    }
    ...
    for (let i = 0; i < cmds.length; i++) {
      ...
      try {
        ...
        // Execute command
        const execResult = await withTimeout<string>(
          opts.winrm.command.doExecuteCommand(params),
          cmdTimeout,
          `Command ${i + 1}/${cmds.length}`,
        );
        ...
        // Get output
        const outputResult = await withTimeout<string>(
          opts.winrm.command.doReceiveOutput(params),
          cmdTimeout,
          `Output ${i + 1}/${cmds.length}`,
        );
 ...
 */
export function winRMBatch(cmds: string[], opts: IWinRMOptions): Promise<string[]> {
  // Check required parameters
  if (!Array.isArray(cmds) || cmds.length === 0) {
  ...
  try {
    ...
    return Promise.resolve(Array.from({ length: cmds.length }).fill('') as string[]);
  }
}
 ...
 */
export function winRMWorkload(cmd: string, opts: IWinRMOptions): Promise<string> {
  if (!cmd || cmd.trim() === '') {
 ...
 */
export async function executeSingleCommand(cmd: string, opts: IWinRMOptions): Promise<string> {
  if (!cmd || cmd.trim() === '') {
    return '';
  }
