...
/**
 * Parse Windows controller information
 * @param sections Controller sections
 * @param vections VRAM sections
 * @returns Array of graphics controllers
 */
function parseLinesWindowsControllers(
  sections: string[],
  vections: string[],
): IGraphicsController[] {
  const memorySizes: Record<string, number> = {};

  for (const vection of vections) {
    if (vection.trim() !== '') {
      const lines = vection.trim().split('\n');
      const matchingDeviceId = util
        .getValue(lines, 'MatchingDeviceId')
        .match(
          /pci\\(ven_[\da-f]{4})&(dev_[\da-f]{4})(?:&(subsys_[\da-f]{8}))?(?:&(rev_[\da-f]{2}))?/i,
        );

      if (matchingDeviceId) {
        const quadWordmemorySize = Number.parseInt(
  ...
  for (const section of sections) {
    if (section.trim() !== '') {
      ...
      // Create a helper function to handle Windows PowerShell output format
      const getValueFromPowerShell = (lines: string[], property: string): string => {
        for (const line of lines) {
          const parts = line.split(':');
      ...
      const vendor = getValueFromPowerShell(lines, 'AdapterCompatibility');
      const model = getValueFromPowerShell(lines, 'Name');
      const pnpDeviceId = getValueFromPowerShell(lines, 'PNPDeviceID');
      const adapterRAM = getValueFromPowerShell(lines, 'AdapterRAM');
      const videoMemoryType = getValueFromPowerShell(lines, 'VideoMemoryType');

      const pnpDeviceIdMatch = pnpDeviceId.match(
        /pci\\(ven_[\da-f]{4})&(dev_[\da-f]{4})(?:&(subsys_[\da-f]{8}))?(?:&(rev_[\da-f]{2}))?/i,
      );
      ...
      if (pnpDeviceIdMatch) {
        subDeviceId = pnpDeviceIdMatch[3] || '';
        ...
        if (memorySize == null && pnpDeviceIdMatch[3] && pnpDeviceIdMatch[4]) {
          const deviceId =
            pnpDeviceIdMatch[1].toUpperCase() +
            '&' +
            pnpDeviceIdMatch[2].toUpperCase() +
            '&' +
            pnpDeviceIdMatch[3].toUpperCase() +
            '&' +
            pnpDeviceIdMatch[4].toUpperCase();
        ...
        if (memorySize == null && pnpDeviceIdMatch[3]) {
          const deviceId =
            pnpDeviceIdMatch[1].toUpperCase() +
            '&' +
            pnpDeviceIdMatch[2].toUpperCase() +
        ...
        if (memorySize == null && pnpDeviceIdMatch[4]) {
          const deviceId =
            pnpDeviceIdMatch[1].toUpperCase() +
            '&' +
            pnpDeviceIdMatch[2].toUpperCase() +
        ...
        if (memorySize == null) {
          const deviceId =
            pnpDeviceIdMatch[1].toUpperCase() + '&' + pnpDeviceIdMatch[2].toUpperCase();
      ...
      controllers.push({
        vendor,
        model,
        bus: pnpDeviceId.startsWith('PCI') ? 'PCI' : '',
        vram: (memorySize == null ? util.toInt(adapterRAM) : memorySize) / 1024 / 1024,
        vramDynamic: videoMemoryType === '2',
        subDeviceId,
      });

      // Update global resolution tracking
      const tempX = util.toInt(getValueFromPowerShell(lines, 'CurrentHorizontalResolution'));
      const tempY = util.toInt(getValueFromPowerShell(lines, 'CurrentVerticalResolution'));
      const tempRefresh = util.toInt(getValueFromPowerShell(lines, 'CurrentRefreshRate'));
      const tempDepth = util.toInt(getValueFromPowerShell(lines, 'CurrentBitsPerPixel'));

      // Only update if we get valid values (not zero)
      if (tempX) {
        _resolutionX = tempX;
      }
      ...
      if (tempRefresh) {
        _refreshRate = tempRefresh;
      }

      if (tempDepth) {
        _pixelDepth = tempDepth;
      }
    }
  }
