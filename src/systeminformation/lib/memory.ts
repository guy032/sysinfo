...
const OSX_RAM_manufacturers: Record<string, string> = {
...
const LINUX_RAM_manufacturers: Record<string, string> = {
  '017A': 'Apacer',
  '0198': 'HyperX',
  '029E': 'Corsair',
  '04CB': 'A-DATA',
 ...
 */
function mem(options: any = {}, callback?: (data: MemData) => void): Promise<MemData> {
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
          ...
          exec(
            ...
            (error, stdout) => {
              if (!error) {
                ...
                const inactive =
                  Number.parseInt(util.getValue(lines, 'vm.stats.vm.v_inactive_count'), 10) *
                  pagesize;
                const cache =
                  Number.parseInt(util.getValue(lines, 'vm.stats.vm.v_cache_count'), 10) * pagesize;
      ...
      if (platform._darwin) {
        ...
        try {
          exec('vm_stat 2>/dev/null | grep "Pages active"', (error, stdout) => {
            if (!error) {
              const lines = stdout.toString().split('\n');
      ...
      if (platform._windows) {
        try {
          const workload: Array<Promise<string>> = [
            util.powerShell(
            ...
            util.powerShell(
              'Get-CimInstance Win32_PerfFormattedData_PerfOS_Memory | Select CacheBytes,ModifiedPageListBytes,StandbyCacheNormalPriorityBytes,StandbyCacheCoreBytes',
              options,
            ) as Promise<string>,
          ];
...
function getManufacturerDarwin(manId: string): string {
...
function getManufacturerLinux(manId: string): string {
...
function memLayout(
  options: any = {},
  callback?: (data: MemLayoutData[]) => void,
): Promise<MemLayoutData[]> {
  const platform: PlatformFlags = getPlatformFlagsFromOptions(options);
              ...
              for (const device of devices) {
                ...
                if (Number.parseInt(util.getValue(lines, 'Size'), 10) > 0) {
                  ...
                  result.push({
                    ...
                    voltageConfigured:
                      Number.parseFloat(util.getValue(lines, 'Configured Voltage:')) || null,
            ...
            if (result.length === 0) {
              ...
              try {
                ...
                if (util.isRaspberry(lines)) {
                  const clockSpeed: Record<string, number> = {
                    0: 400,
                    1: 450,
                    2: 450,
                    3: 3200,
                  ...
                  const freq =
                    Number.parseInt(util.getValue(lines, 'sdram_freq', '=', true), 10) || 0;
                ...
                for (const device of devices) {
                  ...
                  if (size) {
                    result.push({
                      size,
                      bank:
                        util.getValue(lines, 'BankLabel', ':') + (tagInt[1] ? '/' + tagInt[1] : ''), // BankLabel
                      type: memoryTypes[
                        Number.parseInt(util.getValue(lines, 'MemoryType', ':'), 10) ||
                          Number.parseInt(util.getValue(lines, 'SMBIOSMemoryType', ':'), 10)
                      ],
                      ecc: dataWidth && totalWidth ? totalWidth > dataWidth : false,
                      clockSpeed:
                        Number.parseInt(util.getValue(lines, 'ConfiguredClockSpeed', ':'), 10) ||
                        Number.parseInt(util.getValue(lines, 'Speed', ':'), 10) ||
                        0,
                      formFactor:
                        FormFactors[
                          Number.parseInt(util.getValue(lines, 'FormFactor', ':'), 10) || 0
                        ],
                      manufacturer: util.getValue(lines, 'Manufacturer', ':'),
                      partNum: util.getValue(lines, 'PartNumber', ':'),
                      serialNum: util.getValue(lines, 'SerialNumber', ':'),
                      voltageConfigured:
                        (Number.parseInt(util.getValue(lines, 'ConfiguredVoltage', ':'), 10) || 0) /
                        1000,
                      voltageMin:
                        (Number.parseInt(util.getValue(lines, 'MinVoltage', ':'), 10) || 0) / 1000,
                      voltageMax:
                        (Number.parseInt(util.getValue(lines, 'MaxVoltage', ':'), 10) || 0) / 1000,
                    });
                  }
                }
              }
