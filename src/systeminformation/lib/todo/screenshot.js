    ...
    process.nextTick(async () => {
      try {
        ...
        if (options.usePsexec) {
          screenshotCmd = `psexec -i ${sessionId} -u ${username} powershell.exe -NoProfile -EncodedCommand ${encodedCommand}`;
        } else {
          screenshotCmd = psScript;
        }
        
        console.log('Using psexec:', !!options.usePsexec);
...
function parseSessionOutput(output) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const cleanLine = isActive ? line.substring(1).trim() : line;
...
function findActiveSessions(sessions) {
  ...
  return [
    ...activeSessionsByPriority.active,
  ];
}
