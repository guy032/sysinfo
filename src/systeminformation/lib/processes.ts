...
// Define interface for process information
...
// Define interface for process list
...
// Define options interface for processes function
...
function parseTimeUnix(time: string): string {
  let result = time;
  ...
  if (parts.length === 5) {
    result =
      ...
      (
        '0' +
        ('JANFEBMARAPRMAYJUNJULAUGSEPOCTNOVDEC'.indexOf(parts[1].toUpperCase()) / 3 + 1)
      ).slice(-2) +
      '-' +
      ('0' + parts[2]).slice(-2) +
      ' ' +
...
function parseElapsedTime(etime: string): string {
...
function calcProcStatWin(procStat: any, all: number, _cpu_old: any): any {
  // calc
  let cpuu = 0;
  let cpus = 0;
...
function parseProcStat(line: string): number {
...
function calcProcStatLinux(line: string, all: number, _cpu_old: any): any {
...
function parseCommand(fullCommand: string): { command: string; params: string; path: string } {
  let command = '';
  let params = '';
  let path = '';
 ...
 */
function getProcessesScriptPath(): string {
 ...
 */
async function fetchAllProcesses(options: ProcessesOptions = {}): Promise<ProcessInfo[]> {
...
function parseProcessLine(line: string, parsedHead: any, platformFlags: any): ProcessInfo | null {
  let offset = 0;
  let offset2 = 0;

  // Helper to extract column value
  function getColumnValue(i: number, defaultVal = ''): string {
    if (!parsedHead[i]) {
      return defaultVal;
    }
...
async function calculateLinuxCPUStats(result: ProcessesData): Promise<ProcessesData> {
...
async function getUnixProcesses(platformFlags: any, result: ProcessesData): Promise<ProcessesData> {
  return new Promise((resolve) => {
