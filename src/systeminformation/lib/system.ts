 ...
 */
function cleanDefaults(s: string | undefined): string {
  if (!s) {
    return '';
  }
      ...
      if (platform._linux || platform._freebsd || platform._openbsd || platform._netbsd) {
        exec(
          ...
          function (error, stdout) {
            ...
            if (
              (platform._freebsd || platform._openbsd || platform._netbsd) &&
              !result.virtualHost
            ) {
              try {
              ...
              util
                ...
                .then((stdout) => {
                  ...
                  if (result.virtual) {
                    ...
                    util
                      .powerShell(
                        'Get-CimInstance Win32_bios | select Version, SerialNumber, SMBIOSBIOSVersion | fl',
                        options,
                      )
                      .then((stdout) => {
                        const lines = stdout ? stdout.toString().split('\r\n') : [];
...
export function bios(
  options: Options = {},
  callback?: (data: BiosInfo) => void,
): Promise<BiosInfo> {
  const platform = getPlatformFlagsFromOptions(options);

  return new Promise((resolve) => {
    process.nextTick(() => {
      const result: BiosInfo = {
        vendor: '',
        version: '',
        releaseDate: '',
        revision: '',
      ...
      if (platform._windows) {
        try {
          ...
          util
            .powerShell(
              'Get-CimInstance Win32_bios | select Description,Version,Manufacturer,ReleaseDate,BuildNumber,SerialNumber,SMBIOSBIOSVersion | fl',
              options,
            )
            .then((stdout) => {
              const lines = stdout ? stdout.toString().split('\r\n') : [];
...
export function baseboard(
  options: Options = {},
  callback?: (data: BaseboardInfo) => void,
): Promise<BaseboardInfo> {
  const platform = getPlatformFlagsFromOptions(options);

  return new Promise((resolve) => {
    process.nextTick(() => {
      const result: BaseboardInfo = {
        manufacturer: '',
        model: '',
        version: '',
        serial: '-',
      ...
      if (platform._linux || platform._freebsd || platform._openbsd || platform._netbsd) {
        ...
        const getBoardInfo = execPromise(cmd, {}).then((result: any) => ({
          stdout: result.stdout,
          stderr: result.stderr || '',
        }));
        const getMemoryInfo = execPromise(
          'export LC_ALL=C; dmidecode -t memory 2>/dev/null',
          {},
        ).then((result: any) => ({ stdout: result.stdout, stderr: result.stderr || '' }));
        ...
        util.promiseAll(workload).then((data) => {
          ...
          try {
            lines = execSync(cmd, util.execOptsLinux).toString().split('\n');
            result.manufacturer = cleanDefaults(
              result.manufacturer || util.getValue(lines, 'board_vendor'),
            );
            ...
            result.assetTag = cleanDefaults(
              result.assetTag || util.getValue(lines, 'board_asset_tag'),
            );
          } catch {
            util.noop();
          }
      ...
      if (platform._darwin) {
        const execPromise = util.promisify(exec);
        const workload = [
          execPromise('ioreg -c IOPlatformExpertDevice -d 2', {}).then((result: any) => ({
            stdout: result.stdout,
            stderr: result.stderr || '',
          })),
          execPromise('system_profiler SPMemoryDataType', {}).then((result: any) => ({
            stdout: result.stdout,
            stderr: result.stderr || '',
          })),
        ];
      ...
      if (platform._windows) {
        try {
          ...
          const workload: Array<Promise<string | string[]>> = [
            ...
            util.powerShell(
              `Get-CimInstance Win32_physicalmemoryarray | select ${maxCapacityAttribute}, MemoryDevices | fl`,
              options,
            ),
          ];
          util.promiseAll(workload).then((data) => {
...
export function chassis(
  options: Options = {},
  callback?: (data: ChassisInfo) => void,
): Promise<ChassisInfo> {
  const platform = getPlatformFlagsFromOptions(options);
    ...
    process.nextTick(() => {
      ...
      if (platform._linux || platform._freebsd || platform._openbsd || platform._netbsd) {
        ...
        exec(cmd, function (error, stdout) {
          ...
          const ctype = Number.parseInt(
            util.getValue(lines, 'chassis_type').replaceAll(/\D/g, ''),
            10,
          );
        ...
        try {
          util
            ...
            .then((stdout) => {
              ...
              result.model = cleanDefaults(util.getValue(lines, 'model', ':'));
              const ctype = Number.parseInt(
                util.getValue(lines, 'ChassisTypes', ':').replaceAll(/\D/g, ''),
                10,
              );
              result.type =
                ctype && !Number.isNaN(ctype) && ctype < chassisTypes.length
