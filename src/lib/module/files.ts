 ...
 */
function getFilesScriptPath(): string {
  // Calculate the path to the PowerShell script relative to this file
  return path.resolve(__dirname, '../../powershell/files.ps1');
}
...
async function fetchDirectoryData(
  ...
  try {
    ...
    const scriptOptions = {
      ...
      includeSystemDirs:
        batchSettings.includeSystemDirs || DEFAULT_BATCH_SETTINGS.includeSystemDirs,
    };
    ...
    if (
      ...
      const completedChildren: IFileData[] = [];
          ...
          try {
            ...
            if (batchOutput && batchOutput.trim()) {
              ...
              if (nextBatchData.Children && nextBatchData.Children.length > 0) {
                ...
                const processedChildren: IFileData[] = [];
                ...
                for (const child of nextBatchData.Children) {
                  try {
                    ...
                    if (currentDepth < maxDepth - 1) {
                      try {
                        // Recursively process each child directory
                        const fullChildData = await fetchDirectoryData(
                          options,
                          child.Path,
                          currentDepth + 1,
                        );
            ...
            console.error(
              `Error fetching directory batch for ${dirPath} at offset ${dirOffset}:`,
              error,
            );
            hasMoreChildren = false;
          }
    ...
    if (dirData.HasMoreFiles) {
      try {
        ...
        const allFiles: IFile[] = [...(dirData.Files || [])];
        ...
        while (
          ...
          try {
            ...
            console.error(
              `Error fetching file batch for ${dirPath} at offset ${fileOffset}:`,
              error,
            );
            hasMoreFiles = false;
          }
 ...
 */
export function files(options: IFileOptions = {}, callback?: IFilesCallback): Promise<IFileData> {
  ...
  console.log(
    `Starting directory scan on ${startPath} with batch settings:`,
    JSON.stringify({
  ...
  let result: IFileData = {} as IFileData;
    ...
    process.nextTick(async () => {
      try {
        ...
        try {
          ...
          console.log(
            `Completed directory scan. Found ${result.TotalDirCount} directories and ${result.FileCount} files at the root level.`,
          );
          ...
          const cleanChildren = (children: IFileData[]) => {
