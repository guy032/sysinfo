...
function inetChecksite(
  url: string,
  callback?: InetChecksiteCallback,
): Promise<InetChecksiteResult> {
  // Get platform flags from options if provided

  return new Promise((resolve) => {
    process.nextTick(() => {
      const result: InetChecksiteResult = {
        url,
        ok: false,
        status: 404,
        ms: null,
...
function inetLatency(
  options: InetOptions = {},
  host?: string | InetLatencyCallback,
  callback?: InetLatencyCallback,
): Promise<number | null> {
  // Get platform flags from options if provided
  ...
  if (util.isFunction(host) && !callback) {
    callback = host as InetLatencyCallback;
    host = '';
  }

  const hostStr = (host as string) || '8.8.8.8';
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      let hostSanitized = '';
      const s = (
        util.isPrototypePolluted() ? '8.8.8.8' : util.sanitizeShellString(hostStr, true)
      ).trim();
      const l = Math.min(s.length, 2000);
        ...
        util.execSafe('ping', params).then((stdout) => {
          ...
          if (stdout) {
            const lines = stdout
              ...
              .filter(
                (line) =>
                  line.includes('rtt') || line.includes('round-trip') || line.includes('avg'),
              )
              .join('\n');
