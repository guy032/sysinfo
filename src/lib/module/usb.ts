...
function getLinuxUsbType(type: string, name: string): string {
  let result = type;
...
function parseLinuxUsb(usb: string): IUsbDevice {
  const result: IUsbDevice = {
    bus: null,
    deviceId: null,
    id: null,
    name: null,
...
function getDarwinUsbType(name: string): string {
  let result = '';
...
interface IDarwinUsbObject {
  'Built-In'?: string;
  'non-removable'?: string;
  'USB Address'?: string;
  kUSBProductString?: string;
  'USB Product Name'?: string;
  kUSBVendorString?: string;
  'USB Vendor Name'?: string;
  kUSBSerialNumberString?: string;
  [key: string]: any;
}

function parseDarwinUsb(usb: string): IUsbDevice | null {
  const result: IUsbDevice = {
    bus: null,
    deviceId: null,
    id: null,
  ...
  try {
    ...
    const usbObj: IDarwinUsbObject = JSON.parse(lines.join('\n'));
...
function getUsbScriptPath(): string {
  // Calculate the path to the PowerShell script relative to this file
  return path.resolve(__dirname, '../../powershell/usb.ps1');
}

function getWindowsUsbTypeCreation(creationclass: string, name: string): string {
  let result = '';
...
function parseWindowsUsb(lines: string, id: number): IUsbDevice | null {
  const usbType = getWindowsUsbTypeCreation(
    util.getValue(lines.split('\n'), 'CreationClassName', ':').toLowerCase(),
    util.getValue(lines.split('\n'), 'name', ':').toLowerCase(),
  );
...
function usb(options: IUsbOptions = {}, callback?: IUsbCallback): Promise<IUsbDevice[]> {
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (platform._windows) {
        util
          .powerShell(
            'Get-CimInstance CIM_LogicalDevice | where { $_.CimClass -match \\"USB\\" } | select Name,CreationClassName,DeviceId,Manufacturer | fl',
            options,
          )
          .then((stdout) => {
            if (stdout) {
