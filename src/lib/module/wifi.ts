...
// Interface for Linux wireless interface data
interface ILinuxWifiIface {
  id: number;
  iface: string;
  mac: string;
  channel: number;
...
// Interface for NMI device data
interface INmiDeviceInfo {
  iface?: string;
  type?: string;
  vendor?: string;
  product?: string;
...
// Interface for NMI connection data
interface INmiConnectionInfo {
  ssid?: string | null;
  uuid?: string;
  type?: string;
  autoconnect?: boolean;
...
// Interface for WPA connection data
interface IWpaConnectionInfo {
  ssid?: string;
  uuid?: string;
  security?: string;
  freq?: number;
...
// Frequency map
const _wifi_frequencies: Record<number, number> = {
  1: 2412,
  2: 2417,
  3: 2422,
  4: 2427,
 ...
 */
function wifiDBFromQuality(quality: number | string): number {
  const qual = Number.parseFloat(quality as string);
 ...
 */
function wifiQualityFromDB(db: number | string): number {
  const result = 2 * (Number.parseFloat(db as string) + 100);
 ...
 */
function wifiFrequencyFromChannel(channel: number | string): number | null {
 ...
 */
function wifiChannelFromFrequency(frequency: number): number {
  let channel = 0;

  for (const key in _wifi_frequencies) {
    if (
      Object.prototype.hasOwnProperty.call(_wifi_frequencies, key) &&
 ...
 */
function ifaceListLinux(): ILinuxWifiIface[] {
  const result: ILinuxWifiIface[] = [];
 ...
 */
function nmiDeviceLinux(iface: string): INmiDeviceInfo {
 ...
 */
function nmiConnectionLinux(ssid: string): INmiConnectionInfo {
 ...
 */
function wpaConnectionLinux(iface: string): IWpaConnectionInfo {
  if (!iface) {
    return {};
  }
 ...
 */
function getWifiNetworkListNmi(): IWifiNetwork[] {
  const result: IWifiNetwork[] = [];
  const cmd =
 ...
 */
function getWifiNetworkListIw(iface: string): IWifiNetwork[] | -1 {
  const result: IWifiNetwork[] = [];

  try {
    const iwlistParts = execSync(
    ...
    if (iwlistParts.length > 1) {
      ...
      for (const element of iwlistParts) {
        ...
        const address =
          lines && lines.length > 0 && lines[0].includes('Address:')
 ...
 * @returns Array of wifi networks
 */
function parseWifiDarwinXX(wifiObj: any[]): IWifiNetwork[] {
  const result: IWifiNetwork[] = [];
 ...
 * @returns Array of wifi networks
 */
function parseWifiDarwin(wifiStr: string): IWifiNetwork[] {
  const result: IWifiNetwork[] = [];
  ...
  try {
    ...
    for (const wifiItem of wifiObj) {
      ...
      const channel =
        Number.parseInt(String(wifiItem.spairport_network_channel).split(' ')[0], 10) || 0;
...
export function wifiNetworks(
  options: IWifiOptions = {},
  callback?: (data: IWifiNetwork[]) => void,
): Promise<IWifiNetwork[]> {
  const platformFlags = getPlatformFlagsFromOptions(options);

  return new Promise((resolve) => {
    process.nextTick(() => {
      let result: IWifiNetwork[] = [];
          ...
          try {
            ...
            if (iface) {
              ...
              if (res === -1) {
                ...
                const timeoutCallback = function (iface: string) {
...
export function wifiConnections(
  options: IWifiOptions = {},
  callback?: (data: IWifiConnection[]) => void,
): Promise<IWifiConnection[]> {
  const platformFlags = getPlatformFlagsFromOptions(options);

  return new Promise((resolve) => {
    process.nextTick(() => {
      const result: IWifiConnection[] = [];
      ...
      if (platformFlags._linux) {
        ...
        for (const ifaceDetail of ifaces) {
          ...
          const channel =
            network && network.length > 0 && network[0].channel
              ? network[0].channel
              : wpaDetails.channel || null;
          const bssid =
            network && network.length > 0 && network[0].bssid
              ? network[0].bssid
              : wpaDetails.bssid || null;
          const signalLevel =
            network && network.length > 0 && network[0].signalLevel ? network[0].signalLevel : null;
        ...
        exec(cmd, function (error, stdout) {
          try {
            ...
            const networkWifiObj = networkObj.find((item: any) => item._name === 'Wi-Fi');
            const airportWifiObj = airportObj[0].spairport_current_network_information;

            const channel =
              Number.parseInt(String(airportWifiObj.spairport_network_channel).split(' ')[0], 10) ||
              0;
            ...
            result.push({
              ...
              security,
              signalLevel: signalLevel ? Number.parseInt(signalLevel as string, 10) : null,
              quality: wifiQualityFromDB(signalLevel || 0),
        ...
        util.powerShell(cmd, options).then(function (stdout) {
          ...
          for (const part of parts) {
            ...
            if (lines.length >= 5) {
              ...
              for (const line of lines) {
                ...
                if (parts.length > 1) {
                  ...
                  if (lineLower.startsWith('ssid')) {
                    ...
                    lineLower.startsWith('receive rate') ||
              ...
              let txRateValue: number | null = null;

              if (txRate) {
                txRateValue = Number.parseFloat(
                  txRate.replace('Mbps', '').replace('MBit/s', '').trim(),
                );
              }

              let rxRateValue: number | null = null;

              if (rxRate) {
                rxRateValue = Number.parseFloat(
                  rxRate.replace('Mbps', '').replace('MBit/s', '').trim(),
                );
              }
...
export function wifiInterfaces(
  options: IWifiOptions = {},
  callback?: (data: IWifiInterface[]) => void,
): Promise<IWifiInterface[]> {
  const platformFlags = getPlatformFlagsFromOptions(options);

  return new Promise((resolve) => {
    process.nextTick(() => {
      const result: IWifiInterface[] = [];
