...
// Define interfaces for options and return types
 ...
 * @returns Socket type
 */
function getSocketTypesByName(str: string): string {
  let result = '';
 ...
 */
function cpuManufacturer(str: string): string {
  let result = str;
  str = str.toLowerCase();
 ...
 */
function cpuBrandManufacturer(res: { brand: string; manufacturer: string }): {
  brand: string;
  manufacturer: string;
} {
  res.brand = res.brand
 ...
 */
function getAMDSpeed(brand: string): number {
  let result = '0';
 ...
 */
function getCpuCurrentSpeedSync(options: CPUOptions): CPUSpeed {
 ...
 */
export function cpuFlags(
  options: CPUOptions = {},
  callback?: (data: string) => void,
): Promise<string> {
  const { _windows, _linux, _darwin, _freebsd, _openbsd, _netbsd, _sunos } =
...
export function cpuTemperature(
  options: CPUOptions = {},
  callback?: (data: CPUTemperature) => void,
): Promise<CPUTemperature> {
  const { _linux } = getPlatformFlagsFromOptions(options);

  return new Promise((resolve) => {
    process.nextTick(() => {
      const result: CPUTemperature = {
        main: null,
        cores: [],
        max: null,
        socket: [],
 ...
 */
function parseWinCache(linesProc: string, linesCache: string): CPUCache {
  const result: CPUCache = {
    l1d: null,
    l1i: null,
    l2: null,
    l3: null,
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_linux) {
        ...
        try {
          const workload: Array<Promise<string | string[]>> = [
            util.powerShell(
 ...
 */
function getCpu(options: CPUOptions = {}): Promise<CPUInfo> {
  const { _windows, _linux, _darwin, _freebsd, _openbsd, _netbsd, _sunos } =
    getPlatformFlagsFromOptions(options);
      ...
      if (_linux) {
        ...
        exec(
          ...
          function (error, stdout) {
            ...
            result.speedMin =
              Math.round(
                Number.parseFloat(util.getValue(lines, 'cpu min mhz').replaceAll(',', '.')) / 10,
              ) / 100;
            result.speedMax =
              Math.round(
                Number.parseFloat(util.getValue(lines, 'cpu max mhz').replaceAll(',', '.')) / 10,
              ) / 100;
          ...
          function (error, stdout) {
            ...
            for (const element of cache) {
              ...
              const cacheTypeArray = util
                ...
                .replace(' ', '-')
                .split('-');
              const cacheType: string = cacheTypeArray.length > 0 ? cacheTypeArray[0] : '';
      ...
      if (_windows) {
        try {
          ...
          Promise.all(workload).then((data) => {
            ...
            result.speedMax =
              Math.round(
                Number.parseFloat(util.getValue(lines, 'maxclockspeed', ':').replaceAll(',', '.')) /
                  10,
              ) / 100;
 ...
 */
function getLoad(): Promise<CurrentLoadInfo> {
  return new Promise((resolve) => {
    process.nextTick(() => {
      let _corecount = 0;
      let _current_cpu = {
 ...
 * @returns Promise resolving to current load
 */
export function currentLoad(callback?: (data: CurrentLoadInfo) => void): Promise<CurrentLoadInfo> {
  return new Promise((resolve) => {
    process.nextTick(() => {
      getLoad().then((result) => {
        if (callback) {
 ...
 */
function getFullLoad(): Promise<number> {
  return new Promise((resolve) => {
    process.nextTick(() => {
      const cpus = os.cpus();
      let totalUser = 0;
 ...
 * @returns Promise resolving to full load
 */
export function fullLoad(callback?: (data: number) => void): Promise<number> {
  return new Promise((resolve) => {
    process.nextTick(() => {
